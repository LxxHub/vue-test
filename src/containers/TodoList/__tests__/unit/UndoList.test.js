...
describe('UndoList组件测试', () => {
  ...
  it('Undolist 参数为[1,2,3], count为3， 且列表有内容, 且存在删除按钮', () => {
    ...
    const countElem = findTestWrapper(wrapper, 'count')
    const listItems = findTestWrapper(wrapper, 'list-item')
    const deleteBtn = findTestWrapper(wrapper, 'delete-button')
  ...
  it('列表被点击，向外触发 change status 事件', () => {
    ...
    const listItem = findTestWrapper(wrapper, 'list-item').at(1)
  ...
  it('输入框失去焦点，向外触发 reset 事件', () => {
    ...
    const listItem = findTestWrapper(wrapper, 'input').at(0)
